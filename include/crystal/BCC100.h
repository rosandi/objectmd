...
class BCC: public Crystalline {
	...
	void Create100() {
		...
		for (int zm=0; zm<zml; zm++) {		
			int nx=(zm%2)?oNx:eNx;
			int ny=(zm%2)?oNy:eNy;
			double ofs=(zm%2)?lattice_constant/2.0:0.0;	
			for (int a=0; a<nx; a++)
				for (int b=0; b<ny; b++) {
					Atoms(n).x= (double)a *lattice_constant+ofs;
					Atoms(n).y= (double)b *lattice_constant+ofs;
					Atoms(n).z=-(double)zm*lattice_constant/2.0;
					n++;
				}
		}
	}
