...
class SysBox {
public:
	double x0, x1; ///< west--east
	double y0, y1; ///< south--north
	double z0, z1; ///< bottom--top
	double lx, ly, lz; ///< length
	double hlx, hly, hlz; ///< half length
...
public:
    ...
    double aux[MAXAUXVAR];  ///< reserved multi-purpose vector
};
...
public:
	...
	void dump(string fname) {
		...
		mdassert(fl.good(), "fail opening file "+fname+" for writing...");
		reset();
		int a;
		while((a=fetch())>=0) {
			fl << a << std::endl;
