...
class RestartSaver:public Detector, public ParallelGadget {
	OMD_FLOAT saving_period;
	OMD_FLOAT elapsed_time;
	OMD_FLOAT next_save;
	string time_unit;
	time_t current_time;
...
public:
	RestartSaver(OMD_FLOAT speriod=-1.0, string tunit="second") {
	...
	void Init(MDSystem* WorkSys) {
		...
		Filename=System->GetRestartFilename();
		...
		assert(saving_period>0.0, "the saving period is undefined! (parameter save.every)");
	...
	void Detect() {
		...
		if(GetRank()==ROOT) {
			time(&current_time);
			OMD_FLOAT dt=difftime(current_time, previous_time);
			if(dt>saving_period) {
				a=1;
				previous_time=current_time;
				NCalls++;
