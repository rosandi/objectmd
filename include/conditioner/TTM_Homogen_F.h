...
private:
	...
	void friction_force() {
		...
		for(int i=0;i<na;i++){
			...
			double vcx=(a->vx-sumvx[i]);
			double vcy=(a->vy-sumvy[i]);
			double vcz=(a->vz-sumvz[i]);
			double vcsq=(vcx*vcx+vcy*vcy+vcz*vcz);
			
			if (vcsq==0.0) continue;
			
			double de=coup*(electron_temperature - a->aux[atemp])*atom_volume;
			double chi=de/vcsq;
